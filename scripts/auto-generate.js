...
const TOPIC = 'アウトドア・キャンプ';

async function getUnsplashUrl(keyword) {
  const q = encodeURIComponent(keyword.replace(/おすすめ|ランキング|比較|TOP5|【.*?】/g, '').trim() || 'lifestyle');
...
async function generateArticle(keyword) {
  ...
  const imgUrl = await getUnsplashUrl(keyword);
...
![${keyword}のイメージ](${imgUrl})

## 結論：おすすめ1位はこれ

（結論を先に200文字で）

[→ Amazonで今すぐ確認する](${amazonUrl})
[→ 楽天で最安値を見る](${rakutenUrl})
...
async function main() {
  ...
  const keywords = ["テントおすすめランキング", "キャンプ道具比較", "登山グッズ", "BBQグリル", "アウトドア服"];
